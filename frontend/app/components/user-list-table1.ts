...
export default class UserListTableComponent extends Component<UserListTableArgs> {
  ...
  async fetchUsers(page: number | undefined, role: string | null, searchValue: string | null) {
    this.loading = true;
    try {
      const response = await this.reduxStore.store.dispatch(
              userManagementApi.endpoints.fetchUsers.initiate({ page, role, searchValue })
            );
...
console.log("Response Data");
  ...
  handleSearch(event: InputEvent) {
    ...
    debounce(this, this.performSearch, 300); // Add a delay of 300ms to prevent excessive API calls
  }

  @action
  performSearch() {
    this.fetchUsers(1, this.role, this.searchQuery); // Always reset to page 1 when performing a search
  }
